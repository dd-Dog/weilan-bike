...
import com.wlcxbj.bike.util.ChString;
import com.wlcxbj.bike.util.ParseDeviceStateInfoUtil;
...
public class AmapUtil {
    ...
    public static MarkerOptions createMarker(Context context, LatLng pos, int resID, String
...
//        markerOption.setFlat(true);
    ...
    public static MarkerOptions createMarker(Context context, DeviceStateInfo info, int resID) {
        markerOption = new MarkerOptions();
        LatLng point = ParseDeviceStateInfoUtil.getLatLng(info);
        LatLonPoint latLonPoint = ParseDeviceStateInfoUtil.getLatLonPoint(info);
        markerOption.position(point);
        markerOption.title("ID:" + info.getSimno()).snippet("位置:未知");
        markerOption.draggable(true);
        markerOption.icon(
                BitmapDescriptorFactory.fromBitmap(BitmapFactory
                        .decodeResource(context.getResources(), resID)));
...
//        markerOption.setFlat(true);
     ...
     * @param tv
     */
    public static void getAddress(Context context, LatLonPoint latLonPoint, final TextView tv) {
        ...
        geocoderSearch.setOnGeocodeSearchListener(new GeocodeSearch.OnGeocodeSearchListener() {
            ...
            public void onRegeocodeSearched(RegeocodeResult regeocodeResult, int code) {
                ...
                Log.e(TAG, "regeocodeResult=" + formatAddress);
                if (TextUtils.isEmpty(formatAddress)) {
                    tv.setText("未知位置");
                } else {
                    tv.setText(formatAddress);
                }
            }
     ...
     * @param tv
     */
    public static void getBuildingAddress(Context context, LatLonPoint latLonPoint, final
    TextView tv) {
        GeocodeSearch geocoderSearch = new GeocodeSearch(context);//传入context
        // 第一个参数表示一个Latlng，第二参数表示范围多少米，第三个参数表示是火系坐标系还是GPS原生坐标系
        RegeocodeQuery query = new RegeocodeQuery(latLonPoint, 100, GeocodeSearch.AMAP);
...
//					RouteBusLineItem busline = busStep.getBusLines().get(0);
    ...
    public static LatLonPoint createLatLonPointFromStr(String lat, String lon) {
        if (TextUtils.isEmpty(lat) || TextUtils.isEmpty(lon)) {
            return null;
        }
        return new LatLonPoint(Double.parseDouble(lat), Double.parseDouble(lon));
    }
}
